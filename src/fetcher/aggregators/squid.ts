...
const SquidEstimateSchema = z.object({
  fromAmount:             z.string(),
  toAmount:               z.string(),
  // Real price impact from liquidity (e.g. -0.07 = 0.07%). NOT the slippage tolerance.
  aggregatePriceImpact:   z.coerce.number().optional(),
...
export async function fetchSquid(route: RouteKey, _key: string): Promise<NormalizedQuote[]> {
  ...
  const quote: NormalizedQuote = {
    ...
    // Real liquidity price impact, as positive bps (aggregatePriceImpact is a signed %).
    priceImpactBps: est.aggregatePriceImpact != null ? Math.round(Math.abs(est.aggregatePriceImpact) * 100) : null,

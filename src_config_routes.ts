...
import type { Route, RefreshTier, Asset } from '../types';
...
const TIER1_CORRIDORS: [string, string][] = [
  ...
  ['ethereum', 'blast'], ['ethereum', 'mantle'], ['ethereum', 'sonic'],
  // Group 1H: L2 ↔ Growth
...
const TIER2_CORRIDORS: [string, string][] = [
  ...
  // Tron USDT corridors
  ['tron', 'ethereum'], ['tron', 'bsc'], ['tron', 'arbitrum'], ['tron', 'solana'],
  // Berachain corridors
  ['berachain', 'ethereum'], ['berachain', 'arbitrum'], ['berachain', 'base'],
  // Hyperliquid corridors
  ['hyperliquid', 'ethereum'], ['hyperliquid', 'arbitrum'],
  // More L2 ↔ Growth
  ['arbitrum', 'sonic'], ['arbitrum', 'mantle'], ['arbitrum', 'blast'],
  ['arbitrum', 'scroll'], ['base', 'sonic'], ['base', 'blast'],
...
const T1_AMOUNTS = [1000] as const;
...
export function generateAllRoutes(): Route[] {
  ...
  for (const src of CHAIN_SLUGS) {
    for (const dst of CHAIN_SLUGS) {
      ...
      const assets = tier === 1 ? T1_ASSETS : tier === 2 ? T2_ASSETS : T3_ASSETS;
...
export const ROUTE_COUNT = ALL_ROUTES.length; // Should be 870

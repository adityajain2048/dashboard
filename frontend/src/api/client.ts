...
export interface AggregatorHealth {
  ...
  /** Routes where this aggregator found the single highest-output quote. Absent on older backend versions. */
  wins?: number;
  /** wins as % of all contested routes. Absent on older backend versions. */
  winPct?: number;
}
